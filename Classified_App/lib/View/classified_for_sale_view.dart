...
class ClassifiedForSaleView extends StatelessWidget {

  const ClassifiedForSaleView ({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Consumer<Product>(
              ...
              ListView.builder(
                                  itemCount: product.getProductForSaleList.length *2 ,
                                  padding: const EdgeInsets.all(18.0),
                                  itemBuilder: /*1*/ (context, i) {
                                    print('$i ${product.getProductForSaleList.length}');
                                    if (i.isOdd) return const Divider(); /*2*/

                                    final index = i ~/ 2; /*3*/

                                    return _buildRow(context, product.getProductForSaleList[index]);
                                  }
                              )
            ),
